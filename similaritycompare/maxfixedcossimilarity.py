    ...
    for source in range(len(data)):
        if (sinkinput in data[source]):
            ...
            if (len(set_sourceinfo & iter_sourceinfo_set) == 0):
                ...
                itersection = float(len(set_sourceinfo & iter_sourceinfo_set))
                lenset_sourceinfo = len(set_sourceinfo)
                leniter_sourceinfo_set = len(iter_sourceinfo_set)
                cos = (itersection * itersection * itersection) / (math.sqrt(lenset_sourceinfo * leniter_sourceinfo_set) * lenset_sourceinfo * leniter_sourceinfo_set)
...
with open('maxfixedcossimilarity.csv', 'wb') as csvfile:
